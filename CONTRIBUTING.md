...
`internal/catalog/models.go` decides which bytes may reach which monitor. It is Go source so that every byte is reviewable in a
diff, with no parser between the file and the write.
